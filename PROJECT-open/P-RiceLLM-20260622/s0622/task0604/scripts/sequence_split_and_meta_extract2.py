...
def split_fasta_windows(fasta_path, chromosomes, window_size, overlap, output_dir):
    ...
    if chromosomes:
        requested_chromosomes = list(chromosomes)
    else:
        requested_chromosomes = list(genome.keys())

    rows = []
    for chrom in tqdm(requested_chromosomes, desc='Splitting chromosomes'):
        real_chrom_names = None
        ...
        if real_chrom_names is not None:
...
def extract_meta_rows(meta_csv, assay_titles, biosample_names, output_dir):
    ...
    if filtered.empty:
        raise ValueError(
            "No metadata rows left after filtering. "
            f"assay_titles={assay_titles}, biosample_names={biosample_names}. "
            "Check that data_prepare.sh writes real tissue/sample names into biosample_name."
        )
    ...
    expected_tracks = len(
        filtered[['Assay title', 'strand']].drop_duplicates()
    ) * filtered['biosample_name'].nunique()
    if expected_tracks != len(filtered):
        raise ValueError(
            "Filtered metadata is not a full heads × biosamples grid: "
            f"rows={len(filtered)}, expected={expected_tracks}. "
            f"heads={filtered[['Assay title', 'strand']].drop_duplicates().values.tolist()}, "
            f"biosamples={filtered['biosample_name'].drop_duplicates().tolist()}"
        )
...
def build_and_save_index_stat(args, sequence_csv, meta_csv, fasta_path):
    ...
    if args.chromosomes:
        ...
        real_chrom_names = list(genome.keys())
    ...
    try:
        ...
        expected_tracks = len(stats["counts"]["heads"]) * len(stats["counts"]["biosample_order"])
        actual_tracks = len(stats["counts"]["target_file_name"])
        if expected_tracks != actual_tracks:
            raise ValueError(
                "index_stat would be inconsistent: target_file_name count must equal heads × biosample_order, "
                f"got {actual_tracks} != {len(stats['counts']['heads'])} × {len(stats['counts']['biosample_order'])}. "
                f"heads={stats['counts']['heads']}, biosample_order={stats['counts']['biosample_order']}, "
                f"target_file_name={stats['counts']['target_file_name']}"
            )
        if stats["counts"]["nonzero_mean"] and len(stats["counts"]["nonzero_mean"]) != actual_tracks:
            raise ValueError(
                "index_stat would be inconsistent: nonzero_mean count must equal target_file_name count, "
                f"got {len(stats['counts']['nonzero_mean'])} != {actual_tracks}"
            )
            
    except Exception:
        logging.exception("Failed to build consistent counts for index_stat.json")
        raise
...
def parse_args():
    ...
    p.add_argument('--chromosomes', nargs='*', default=None, help='Chromosomes to process (optional for gene-region FASTA)')
