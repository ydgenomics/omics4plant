...
def _eval_value_to_numpy(value, preferred_keys):
    """Extract the tensor/array HF passes to metrics from dict/tuple wrappers."""
...
def compute_multimodal_metrics(eval_pred: EvalPrediction, val_chromosomes, tokenizer, channel_names=None):
    """
    计算多模态预测的评估指标
    ...
    # 解析模型输出和标签
    preds = _eval_value_to_numpy(eval_pred.predictions, ("logits", "predictions"))  # [B, L, C]
    labels = _eval_value_to_numpy(eval_pred.label_ids, ("labels", "label_ids"))     # [B, L, C]
    ...
    # 旧数据曾在 input_ids 前缀里放 chromosome/biosample；当前数据集只放原始序列。
    # 如果解析不到前缀，就按 all/all 汇总，避免动态通道结构下验证阶段越界。
    filter_by_chrom = True
    try:
        input_chroms = [tokenizer.convert_ids_to_tokens([seq[0]])[0][1:-1] for seq in input_ids]
        input_biosample = [tokenizer.convert_ids_to_tokens([seq[1]])[0][1:-1] for seq in input_ids]
    except Exception:
        input_chroms = ["all"] * len(preds)
        input_biosample = ["all"] * len(preds)
        filter_by_chrom = False
    
    num_channels = preds.shape[-1]
    if channel_names is None or len(channel_names) != num_channels:
        modalities = [f"track_{i}" for i in range(num_channels)]
    else:
        modalities = list(channel_names)
    
    # 存储各模态的结果
    modality_preds = {modality: [] for modality in modalities}
    modality_labels = {modality: [] for modality in modalities}
    
    # 处理预测结果，将 [B, L, C] 张量拆分为 C 个通道
    ...
    for i, (chrom, biosample) in enumerate(zip(input_chroms, input_biosample)):
        if filter_by_chrom and val_chromosomes and chrom not in val_chromosomes:
            continue
        key = (biosample, chrom)
        if key not in group_map:
            group_map[key] = []
