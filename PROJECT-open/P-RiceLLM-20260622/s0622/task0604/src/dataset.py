...
def load_chromosome_features(path, normalize=True):
    ...
    Supported JSON formats:
      1) {"feature_names": [...], "features": {"Chr01": [..], ...}}
      2) {"features": {"Chr01": {"gc_ratio": 0.43, ...}, ...}}
      3) {"Chr01": [..], "Chr03": [..]}
      4) {"Chr01": {"gc_ratio": 0.43, ...}, ...}
    """
    ...
    normalized_rows = {}
    for chrom, vector in zip(rows.keys(), matrix):
...
def sample_viewer(sample):
    ...
    sample_channel_names = sample.get("channel_names", None)
    ...
    if lab.ndim == 1:
        ...
        # 若是 [C, L]，则转置为 [L, C]。当前通道数由 index_stat 决定。
        known_channels = len(sample_channel_names) if sample_channel_names else len(files or [])
        if known_channels and lab.shape[0] == known_channels and lab.shape[1] != known_channels:
            lab = lab.T
    else:
    ...
    # 通道名称优先使用 sample 中给出的动态名称，其次使用 files 或通用名。
    if sample_channel_names and len(sample_channel_names) == channels:
        channel_names = list(sample_channel_names)
    elif files:
...
class MultiTrackDataset(Dataset):
    ...
    def __init__(self, 
        ...
        self.max_length = max_length
        self.mode = mode
        self.chromosome_features = chromosome_features.get("features", chromosome_features) if chromosome_features else None
        self.chromosome_feature_names = chromosome_features.get("feature_names", None) if chromosome_features else None
        self.chromosome_feature_dim = chromosome_features.get("dim", None) if chromosome_features else None
        if self.chromosome_features and self.chromosome_feature_dim is None:
            first_vector = next(iter(self.chromosome_features.values()))
            self.chromosome_feature_dim = len(first_vector)
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            **self._chromosome_feature_item(chromosome_val),
        }

    def _chromosome_feature_item(self, chromosome_val):
        if not self.chromosome_features:
            return {}
        vector = self.chromosome_features.get(str(chromosome_val))
        if vector is None:
            for alias in _chrom_aliases(chromosome_val):
                vector = self.chromosome_features.get(alias)
                if vector is not None:
                    break
        if vector is None:
            if self.chromosome_feature_dim is None:
                return {}
            vector = [0.0] * int(self.chromosome_feature_dim)
        return {
            "chrom_features": torch.tensor(vector, dtype=torch.float32)
        }
