...
def _feature_gene_name(parsed_attrs, transcript_to_gene, prefer_parent=False):
    parent = parsed_attrs.get("Parent", "")
    if prefer_parent and parent:
    ...
    for key in ("gene_name", "Name", "gene_id", "ID"):
        if parsed_attrs.get(key):
            return parsed_attrs[key]
    for parent_id in parent.split(','):
        if parent_id in transcript_to_gene:
            return transcript_to_gene[parent_id]
...
def _feature_transcript_id(parsed_attrs):
    parent = parsed_attrs.get("Parent", "")
    return parent.split(',')[0] if parent else parsed_attrs.get("ID", "")


def _draw_gene_models(ax_gene, genes, exons, cds, transcripts, start_display, end_display,
                      gene_color_plus, gene_color_minus, max_transcripts_per_gene=3):
    if not genes:
    ...
    max_rows_by_level = {lvl: 1 for lvl in range(len(level_ends))}

    for (_, rowg), lvl in zip(genes_df.iterrows(), gene_levels):
        ...
        gene_transcripts = [
            ...
            if t[3] == name and not (int(t[1]) < gs or int(t[0]) > ge)
        ]
        if not gene_transcripts:
            transcript_ids = [name]
            transcript_ranges = {name: (gs, ge)}
        else:
            gene_transcripts = sorted(set(gene_transcripts), key=lambda t: (int(t[0]), int(t[1]), str(t[4])))
            gene_transcripts = gene_transcripts[:max(1, int(max_transcripts_per_gene))]
            transcript_ids = [t[4] for t in gene_transcripts]
            transcript_ranges = {t[4]: (int(t[0]), int(t[1])) for t in gene_transcripts}

        max_rows_by_level[lvl] = max(max_rows_by_level.get(lvl, 1), len(transcript_ids))
        level_offset = sum(max_rows_by_level.get(prev, 1) * transcript_gap + gene_gap for prev in range(lvl))

        for tx_idx, transcript_id in enumerate(transcript_ids):
            tx_start, tx_end = transcript_ranges.get(transcript_id, (gs, ge))
            y = level_height_base + level_offset + tx_idx * transcript_gap
            ax_gene.plot([tx_start, tx_end], [y, y], color=color, lw=1.7, zorder=1, solid_capstyle='round')

            gene_length = max(1, tx_end - tx_start)
            arrow_len = min(gene_length * 0.1, 2000)
            if strand == "+":
                ax_gene.arrow(tx_end - arrow_len, y, arrow_len, 0, head_width=0.035, head_length=arrow_len * 0.3,
                              fc=color, ec=color, linewidth=0, length_includes_head=True, zorder=3)
            else:
                ax_gene.arrow(tx_start + arrow_len, y, -arrow_len, 0, head_width=0.035, head_length=arrow_len * 0.3,
                              fc=color, ec=color, linewidth=0, length_includes_head=True, zorder=3)

            gene_exons = [e for e in exons if e[3] == name and (not transcript_id or e[4] == transcript_id)]
            for es, ee, _st, _nm, _tid in gene_exons:
                es_d, ee_d = max(es, start_display), min(ee, end_display)
                if ee_d > es_d and (ee_d - es_d) > 20:
                    rect = patches.Rectangle((es_d, y - 0.035), ee_d - es_d, 0.07,
                                            facecolor=color, alpha=0.35, zorder=3,
                                            edgecolor=color, linewidth=0.4)
                    ax_gene.add_patch(rect)

            gene_cds = [c for c in cds if c[3] == name and (not transcript_id or c[4] == transcript_id)]
            for cs, ce, _st, _nm, _tid in gene_cds:
                cs_d, ce_d = max(cs, start_display), min(ce, end_display)
                if ce_d > cs_d and (ce_d - cs_d) > 20:
                    rect = patches.Rectangle((cs_d, y - 0.055), ce_d - cs_d, 0.11,
                                            facecolor=color, alpha=0.95, zorder=4,
                                            edgecolor='black', linewidth=0.35)
                    ax_gene.add_patch(rect)

            label = name if tx_idx == 0 else transcript_id
            text_x = np.clip((tx_start + tx_end) / 2, start_display + 500, end_display - 500)
            ax_gene.text(text_x, y + 0.065, label if label else "Unknown", ha='center', va='bottom',
                         fontsize=8, zorder=5,
                         bbox=dict(boxstyle="round,pad=0.18", facecolor='white', alpha=0.8, edgecolor='none'))

    gene_track_height = level_height_base + sum(max_rows_by_level.get(lvl, 1) * transcript_gap + gene_gap for lvl in range(len(level_ends))) + 0.15
    ...
    legend_elems = [
        ...
        patches.Patch(facecolor='gray', alpha=0.35, edgecolor='gray', label='Exon'),
        patches.Patch(facecolor='gray', alpha=0.95, edgecolor='black', label='CDS')
    ]
    ax_gene.legend(handles=legend_elems, loc='upper right', fontsize=8, framealpha=0.9)
...
class ResultsViewer:
            ...
            with open_func(path, mode) as fh:
                for line in fh:
                    ...
                    try:
                        ...
                        continue
                    parsed_attrs = _parse_gff_attrs(attrs)
                    if feature == "gene":
                        gene_name = _feature_gene_name(parsed_attrs, transcript_to_gene)
                        genes.setdefault(chrom, []).append((start_i, end_i, strand, gene_name))
                    elif feature in ("mRNA", "transcript"):
                        transcript_id = parsed_attrs.get("ID", "")
                        gene_name = _feature_gene_name(parsed_attrs, transcript_to_gene, prefer_parent=True)
                        if transcript_id and gene_name:
                            transcript_to_gene[transcript_id] = gene_name
                            transcripts.setdefault(chrom, []).append(
                                (start_i, end_i, strand, gene_name, transcript_id)
                            )
                    elif feature == "exon":
                        gene_name = _feature_gene_name(parsed_attrs, transcript_to_gene, prefer_parent=True)
                        transcript_id = _feature_transcript_id(parsed_attrs)
                        exons.setdefault(chrom, []).append((start_i, end_i, strand, gene_name, transcript_id))
                    elif feature == "CDS":
                        gene_name = _feature_gene_name(parsed_attrs, transcript_to_gene, prefer_parent=True)
                        transcript_id = _feature_transcript_id(parsed_attrs)
                        cds.setdefault(chrom, []).append((start_i, end_i, strand, gene_name, transcript_id))
        except Exception as e:
            logging.warning(f"Failed to load annotation {path}: {e}")
        self.genes_by_chrom = genes
        self.exons_by_chrom = exons
        self.cds_by_chrom = cds
        self.transcripts_by_chrom = transcripts
        logging.info(f"Loaded annotation: chromosomes={list(self.genes_by_chrom.keys())}")

    def get_genes_in_interval(self, chrom, start, end):
        """Return (genes, exons, cds) overlapping [start, end)."""
        genes = self.genes_by_chrom.get(chrom, [])
        exons = self.exons_by_chrom.get(chrom, [])
        cds = self.cds_by_chrom.get(chrom, [])
        transcripts = self.transcripts_by_chrom.get(chrom, [])
        genes_f = [(gs, ge, st, nm) for (gs, ge, st, nm) in genes if not (ge < start or gs >= end)]
        exons_f = [(es, ee, st, nm, tid) for (es, ee, st, nm, tid) in exons if not (ee < start or es >= end)]
        cds_f = [(cs, ce, st, nm, tid) for (cs, ce, st, nm, tid) in cds if not (ce < start or cs >= end)]
        transcripts_f = [
            (ts, te, st, nm, tid)
            for (ts, te, st, nm, tid) in transcripts
            if not (te < start or ts >= end)
        ]
        return genes_f, exons_f, cds_f, transcripts_f
    ...
    def plot(self,
        ...
        # === 获取基因注释 ===
        genes, exons, cds, transcripts = self.get_genes_in_interval(chrom, start_display, end_display)
        ...
        _draw_gene_models(
            ax_gene, genes, exons, cds, transcripts,
            start_display, end_display,
            gene_color_plus, gene_color_minus,
        ...
        for head in track_names:
            ...
            for b in biosamples:
                ...
                if arr is not None:
                    ...
                    # 第一个结果使用实线
                    ax.plot(display_positions, y, color='tab:blue', linewidth=1.5, alpha=0.9, label='突变前' if b == biosamples[0] else "")
                ...
                if results2:
                    ...
                    if arr2 is not None:
                        ...
                        ax.plot(display_positions, y2, color='tab:red', linewidth=1.5, alpha=0.9, 
                            linestyle='--', label='突变后' if b == biosamples[0] else "")
