...
class CustomTrainer(Trainer):
    def __init__(self, *args, **kwargs):
        ...
        self.chrom2id = {f"Chr{i:02d}": i for i in range(1, 13)}
        self.chrom2id.update({f"chr{i}": i for i in range(1, 13)})
        self.chrom2id.update({f"chr{i:02d}": i for i in range(1, 13)})
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        for key in ("position_chrom", "position_start", "position_end", "chrom_features"):
            if key in inputs:
        ...
        if return_outputs:
            # HF evaluation treats the second return value as model outputs and
            # eventually passes logits to compute_metrics. Keep per-head losses
            # only for training_step logging; otherwise a dict can reach metrics
            # as predictions and crash on preds.shape.
            extra = {"logits": outputs.get("logits")}
            if model.training:
    ...
    def _collate_fn(self, batch):
        ...
        position_chrom = torch.tensor([self._chrom_to_id(c) for c in chroms], dtype=torch.long)
        ...
        if "chrom_features" in batch[0]:
            result["chrom_features"] = torch.stack([b["chrom_features"] for b in batch])
        return result

    def _chrom_to_id(self, chrom) -> int:
        chrom = str(chrom)
        if chrom in self.chrom2id:
            return self.chrom2id[chrom]
        import re
...
class LocalLoggerCallback(TrainerCallback):
    def __init__(self, log_file_path: str, metrics_file_path: Optional[str] = None, log_train_metrics_jsonl: bool = True):
        super().__init__()
        self.log_file_path = log_file_path
        self.metrics_file_path = metrics_file_path or log_file_path.replace(".log", "_metrics.jsonl")
        self.train_metrics_file_path = log_file_path.replace(".log", "_train_metrics.jsonl")
        self.log_train_metrics_jsonl = log_train_metrics_jsonl
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        ...
        # --- 写入结构化日志，避免从 stdout 里解析混杂的进度条文本 ---
        is_eval_log = logs and ("eval_loss" in logs or any(k.startswith("eval_") for k in logs.keys()))
        target_path = self.metrics_file_path if is_eval_log else self.train_metrics_file_path
        if logs and (is_eval_log or self.log_train_metrics_jsonl):
            try:
                log_entry = {
                    "step": _step,
