...
option_list <- list(
  ...
  make_option(c("--sample_names"), type="character", default=NULL,
  ...
  make_option(c("--sample_key"), type="character", default="sample",
...
if(is.null(opt$sample_names)) {
    ...
    if(length(sample_names) != n_samples) {
        stop("sample_labels的长度必须与sample_paths相同")
    }
}
...
for (i in 1:n_samples) {
  cat(sprintf("  %d: %s -> %s\n", i, sample_names[i], sample_paths[i]))
}

# 设置计算资源
plan("multicore", workers = opt$n_cores)
ram_bytes <- opt$max_ram * 1024^3  # 转换为字节
options(future.globals.maxSize = ram_bytes)
cat(sprintf("使用 %d 核心，最大内存 %.1f GB\n", opt$n_cores, opt$max_ram))
...
combined_peaks <- combined_peaks[
  peak_widths < opt$max_peak_width & peak_widths > opt$min_peak_width
]
cat(sprintf("  过滤后peak数量: %d (宽度在 %d-%d bp之间)\n", 
            length(combined_peaks), opt$min_peak_width, opt$max_peak_width))
...
for (i in 1:n_samples) {
  ...
  md <- md[md$fragments > opt$min_passed_filters, ] #华大的数据对应的列名为fragments
  cat(sprintf("    过滤后细胞数: %d (fragments > %d)\n", 
              nrow(md), opt$min_passed_filters))
  ...
  # 添加数据集标识
  seurat_obj[[opt$sample_key]] <- sample_name
...
cat("\n各样本细胞数:\n")
print(table(combined[[opt$sample_key]]))

# 步骤5: 保存结果
cat("\n步骤5: 保存结果...\n")

# 保存合并后的Seurat对象
output_file <- file.path(opt$output_dir, paste0(opt$output_prefix, "_combined.rds"))
cat(sprintf("保存合并对象到: %s\n", output_file))
saveRDS(combined, output_file)

# 保存细胞统计信息
stats_file <- file.path(opt$output_dir, paste0(opt$output_prefix, "_cell_stats.csv"))
stats_df <- data.frame(
...
peaks_file <- file.path(opt$output_dir, paste0(opt$output_prefix, "_combined_peaks.bed"))
...
# 可选：生成合并报告
report_file <- file.path(opt$output_dir, paste0(opt$output_prefix, "_report.txt"))
sink(report_file)
cat("样本合并报告\n")
cat("============\n\n")
cat("合并时间:", date(), "\n\n")
...
cat("  最小fragment数:", opt$min_passed_filters, "\n")
cat("  peak宽度范围:", opt$min_peak_width, "-", opt$max_peak_width, "bp\n\n")
cat("结果统计:\n")
