...
def get_parameters():
    ...
    if len(args) != 6:
        print('''
### Usage
python ../get_cistopic_model.py $atac4cistopic_dir $mallet_path $n_cpu $mallet_mem $numTopics $atac_key

### Example
parent_dir=$(pwd)

mkdir -p "$parent_dir"/create_cistopic/get_cistopic_model && cd "$parent_dir"/create_cistopic/get_cistopic_model

mallet_targz="$parent_dir"/create_cistopic/Mallet-202108-bin.tar.gz
tar -zxvf $mallet_targz

prefix="Os"
atac4cistopic_dir="$parent_dir"/input/from_atac/process_atac/${prefix}_rds2cistopic
mallet_path="$parent_dir"/create_cistopic/get_cistopic_model/Mallet-202108/bin/mallet
n_cpu=30
mallet_mem="250G"
numTopics=45
atac_key="sctype"
...
args = get_parameters()
atac4cistopic_dir = args[0]
mallet_path = args[1]
n_cpu = int(args[2])
mallet_mem = args[3]
numTopics = int(args[4])
atac_key = args[5]
