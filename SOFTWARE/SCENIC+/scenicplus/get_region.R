...
# rna.loom <- as.loom(rna, filename = "rna.loom", verbose = TRUE) 
# rna.loom$close_all()
...
writeMM(counts_sparse, file = "atac_peaks_sparse.mtx")
cell_names <- colnames(counts_sparse)
region_names <- rownames(counts_sparse)
cell_names_file <- "atac_cell_names.txt"
region_names_file <- "atac_region_names.txt"
...
write.table(metadata_frame, file = "atac_metadata.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
write.table(t(bed_data), "atac_region_names.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)

library(Biostrings)
seqs <- readDNAStringSet(fa)
chrom_sizes <- data.frame(chr = names(seqs), size = width(seqs))
write.table(chrom_sizes, "chrom.sizes", 
