...
@click.option('--batch_key', type=str, default=None, help="Batch key in identifying HVG and integration")
...
def run_harmony(input_h5ad, out_h5ad, out_umap, batch_key, key_list, cluster_name, resolution):
    ...
    with PdfPages(out_umap) as pdf:
        ...
        if all(col in adata.obs.columns for col in required_cols):
            sc.pl.violin(adata, keys=['total_counts'], log=False, groupby=batch_key, show=False)
            plt.savefig(pdf, format='pdf', dpi=300, bbox_inches='tight')
            plt.close()
            sc.pl.violin(adata, keys=['n_genes'], log=False, groupby=batch_key, show=False)
            plt.savefig(pdf, format='pdf', dpi=300, bbox_inches='tight')
            plt.close()
        else:
